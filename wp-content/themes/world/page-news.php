    ...
    <a href="<?php echo get_template_directory_uri();?>/pageeeee.php">HELLO</a>
        <div>
            <?php
                ...
                $posts = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}news LIMIT 5");
                if ( $posts ) {
                    echo "<h2>Recent Posts:</h2>";
                    foreach ( $posts as $post ) {
                        echo "<a href='";?><?php echo get_template_directory_uri();?><?php echo  "'>"
                        "<div class = 'title'>".$post->title . "</div>" . $post->content . "" .$post->data."</a><br>";
                    }
                }
                echo "</div>";
                echo "</div>";
...
<script>
       function goto(v) {
            $.ajax({
                url: "<?php echo get_template_directory_uri();?>/admin-ajax.php",
                type: 'POST',
                data: { action:v},
